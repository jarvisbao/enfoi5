...
export function setButton(events, bpmnModeler, bpmnElements, isNode) {
  ...
  if (events.allow_script && events.allow_script.length > 0) {
    const pythonElements = moddle.create('enfo:AllowScript', { cls: events.allow_script[0], fun: events.allow_script[1] })
    buttonsElement.allow_script = [pythonElements]
  }
  if (events.submit_script && events.submit_script.length > 0) {
    const submitElements = moddle.create('enfo:SubmitScript', { cls: events.submit_script[0], fun: events.submit_script[1] })
