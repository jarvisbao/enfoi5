...
export default function router_permission(router) {
  NProgress.configure({ showSpinner: false }) // NProgress configuration
  // permission judge function
  const name = process.env.VUE_APP_NAME || null
    ...
    if (getToken()) {
      if (to.path === '/login') {
        ...
        if (!permission_menus) {
          try {
            // 获取用户菜单
            await store.dispatch('GetPermissionMenus', { addon_menu: name === 'mobile' ? null : name, device: 'mobile' }).then((response) => {
              if (response.code !== Constlib.ERROR_CODE_OK) {
                store.dispatch('FedLogOut')
                Message.error(
                  response.message || 'Verification failed, please login again'
