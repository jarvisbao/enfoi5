    ...
    <el-form ref="form" :model="classColumn" :rules="rules" label-width="130px">
      ...
      <el-row>
        ...
        <el-col :span="11" :offset="1">
          <el-form-item label="列表显示后缀" prop="suffix_letter">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        column_type: [{ required: true, message: '请输入栏目类型', trigger: 'change' }],
  ...
  created() {
    ...
    Object.keys(this.props).forEach(key => {
      if (column_props[key] !== undefined) {
        this.props[key] = column_props[key]
      }
    })
    this.$delete(column_props, 'primary_key')
    this.$delete(column_props, 'nullable')
    this.$delete(column_props, 'default')

    this.classColumn.column_type_args = JSON.stringify(this.classColumn.column_type_args)
    this.classColumn.column_props = JSON.stringify(column_props)
    this.classColumn.data_convert = JSON.stringify(this.classColumn.data_convert)
  },
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.loading = true
          const repo_name = this.classColumn.repo_name
          const class_name = this.classColumn.class_name
          const old_column_name = this.old_column_name
          ...
          this.$Apis.class.column_edit(repo_name, class_name, old_column_name, column_name, column_type, column_type_args, column_props, column_comment, data_convert, data_format, suffix_letter, deferred).then(response => {
