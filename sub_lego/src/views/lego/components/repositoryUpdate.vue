...
export default {
  ...
  data() {
    ...
    return {
      rules: {
        db_uri: [
          { required: true, message: '请输入仓库连接的URI', trigger: 'blur' }
        ],
  ...
  created() {
    this.repository.options = JSON.stringify(this.repository.options)
  },
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$Apis.repository.repository_edit(old_name, db_uri, name, options).then(response => {
