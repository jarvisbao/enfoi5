...
<!--        <el-switch v-model="relationship.multiple" />-->
<!--      </el-form-item>-->
...
export default {
  ...
  data() {
    ...
    return {
      rules: {
        ...
        argument: [{ required: true, message: '请输入被映射的类', trigger: 'blur' }],
  ...
  created() {
    this.relationship.props = JSON.stringify(this.relationship.props)
  },
  mounted() {
    this.getList()
  },
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$Apis.class.relationship_edit(repo_name, class_name, old_name, name, argument, props, multiple, comment).then(response => {
