...
export default {
  ...
  created() {
    ...
    if (SSOToken) {
      if (this.islogin === null || this.islogin) {
        this.SSOTokenLogin(SSOToken)
      } else {
        this.SSOTokenBind(SSOToken)
      }
    }
  ...
  methods: {
    ...
    SSOTokenLogin(SSOToken) {
      this.$store.dispatch('SSOTokenLogin', { SSOToken: SSOToken }).then(response => {
        if (response.code === this.$Utils.Constlib.ERROR_CODE_OK) {
          let redirect = this.$route.query.redirect || this.$route.query.redirects
          if (!redirect) {
            redirect = '/'
    ...
    SSOTokenBind(SSOToken) {
      this.$Apis.user.bindSSOToken(SSOToken).then(response => {
        response['_type'] = 'bind'
        window.opener.postMessage(response, '*')
        window.close()
      })
