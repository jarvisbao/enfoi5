  ...
  export default {
    ...
    data() {
      return {
        ...
        wdgtIdArry: [],
        showItems: false,
        // activeName: process.env.VUE_APP_NAME,  //如果代码没有问题应该删除掉
        activeTabs: [],
        handleWidget: {},
    ...
    computed: {
      ...mapGetters(['appconfig', 'vue_app_name']),
      activeName: { //获取|设置当前激活的应用名称，用于tab
        get() {
          try { // 防止用户直接使用路由地址进入或者手动清除sessionStorage 从而导致错误
            return JSON.parse(sessionStorage.getItem('homepageMgr')).activeAppName
          } catch (e) {
            if (e.message == `Cannot read property 'activeAppName' of null`) {
              // eslint-disable-next-line no-console
              console.warn('未从正常入口进入主页管理页面')
            }
            return this.vue_app_name
          }
        },
        set(newVal) {
          sessionStorage.setItem('homepageMgr', JSON.stringify({
            activeAppName: newVal
          }))
        }
    ...
    methods: {
      ...
      initialData() {
        ...
        this.showItems = true
        this.widgetItem = [...this.basicComponents].filter(x => [...this.newWidgetItem].every(y => y.wdgt_id !== x
          .wdgt_id))
      ...
      handlerSave() {
        ...
        this.$Apis.user.user_update(openid, gender, birthday, last_name, first_name, props, head_id, widgets_json).then(
          response => {
            if (response.code === this.$Utils.Constlib.ERROR_CODE_OK) {
              this.$alert('更新成功!', '提示', {
