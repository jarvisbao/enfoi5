<template>
  <div class="widget-container">
    <el-form ref="form" :model="widget" :rules="rules" label-width="150px">
      ...
      <template v-if="widget.wdgt_type!=='0'&&widget.wdgt_type!=='1'">
        <!-- <el-form-item label="针对模块">
          <el-select v-model="widget.app_code" placeholder="请选择">
            <el-option
        ...
        <template v-if="widget.wdgt_type==='条目式'">
          ...
          <el-form-item label="主键字段">
            ...
            <div class="tips">
              点击条目打开链接时，在链接后面添加主键字段的值作为参数，多个字段用 || 隔开
            </div>
          </el-form-item>
          <!-- <el-row>
            <el-col :span="12">
    ...
    <!-- <el-dialog v-if="dialogVisiblePie" :visible.sync="dialogVisiblePie" :title="dialogTitle" width="35%" append-to-body>
      <pie-create v-if="isCreate" @showPie="isShowPie"/>
      <pie-update v-else @showPie="isShowPie"/>
    </el-dialog> -->
...
import { widget_info, widget_edit } from '@/library/api/widget'
import CommonTab from '@/components/commonTab'
import elementIcon from '@/library/ui/iconChoose/elementIcon'
import alifontIcon from '@/library/ui/iconChoose/alifontIcon'
// import pieCreate from './components/pieCreate'
// import pieUpdate from './components/pieUpdate'
...
export default {
  ...
  created() {
    if (JSON.stringify(this.widgetInfo) === '{}') {
      this.fetchData()
    }
  },
  methods: {
    get_wdgt_id() {
      if ('wdgt_id' in this.$route.query) {
        const wdgt_id = this.$route.query.wdgt_id
        return wdgt_id
      } else {
        return null
      }
    ...
    fetchData() {
      const wdgt_id = this.get_wdgt_id()
      widget_info(wdgt_id).then(response => {
        if (response.code === this.$Utils.Constlib.ERROR_CODE_OK) {
          this.widget = response.payload
          switch (this.widget.wdgt_type) {
            case '条目式':
              this.widgetList = JSON.parse(this.widget.content_config)
              break
            case '占比式':
              this.widgetPercent = JSON.parse(this.widget.content_config)
              break
            case '嵌入式':
              this.widgetInset = JSON.parse(this.widget.content_config)
              break
            case '磁贴式':
              this.widgetPaste = JSON.parse(this.widget.content_config)
              break
            case '内联式':
              this.widgetInline = JSON.parse(this.widget.content_config)
              break
            case '饼图':
              this.widgetPie = JSON.parse(this.widget.content_config)
              break
            case '折线或柱形图':
              this.widgetChart = JSON.parse(this.widget.content_config)
          }
          if (this.widget.wdgt_icon) {
            this.showIcon = true
          } else {
            this.showIcon = false
          }
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const wdgt_id = this.widget.wdgt_id
          ...
          if (this.widget.wdgt_type !== '0' && this.widget.wdgt_type !== '1') {
            content_config = JSON.stringify(this.widget.content_config)
          }
          ...
          this.loading = true
          widget_edit(wdgt_id, this.openid, wdgt_title, wdgt_color, wdgt_icon, wdgt_type, app_code, content_url, content_prop, content_config, more_url, wdgt_summary, mob_content_url).then(response => {
            this.loading = false
            if (response.code === this.$Utils.Constlib.ERROR_CODE_OK) {
              this.$alert('修改成功!', '提示', {
