<template>
  ...
  >
    ...
    <div v-if="selectWidget.key == element.key" class="widget-view-action widget-col-action">
      <i class="iconfont-fm icon-icon_clone" @click.stop="handleTabClone(index)" title="复制" />
  ...
  methods: {
    ...
    handleWidgetTabAdd($event, element, tabIndex) {
      ...
      this.$set(element.tabs[tabIndex].list, newIndex, {
        ...
        name_model: { 'label': eval('element.tabs[tabIndex].list[newIndex].name'), 'prop': key }
      })

      if (element.tabs[tabIndex].list[newIndex].type == 'report') {
        element.tabs[tabIndex].list[newIndex].rows = this.$Utils.util.generateKeyToTD(element.tabs[tabIndex].list[newIndex].rows)
      }
