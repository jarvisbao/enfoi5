...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      this.$Apis.widget.widget_info(wdgt_id).then(response => {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.loading = true
          this.$Apis.widget.widget_edit(wdgt_id, this.openid, wdgt_title, wdgt_color, wdgt_icon, wdgt_type, app_code, content_url, content_prop, content_config, more_url, wdgt_summary, mob_content_url).then(response => {
            this.loading = false
            if (response.code === this.$Utils.Constlib.ERROR_CODE_OK) {
              this.$alert('修改成功!', '提示', {
